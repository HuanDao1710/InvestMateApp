...
import {Text, View, StyleSheet, ScrollView, Dimensions, Modal, TouchableOpacity, TouchableHighlight, Alert} from 'react-native';
...
const GroupStock = (props: {item : WatchlistEntity, navigation : any, deleteWatchlist: any, editWatchlist: any, handleEdit : any,}) => {
    const [numStock, setNumStock] = useState<number>(0);
    const [listTrackingStock, setListTrackingStock] = useState<TrackingStockEntity[]> ([]);
    ...
    const handleDeleteWatchlist = () => {
        Alert.alert(
            ...
            [
                {text: "Huỷ" ,onPress:()=>{}},
                {text: "Xoá", onPress: () => {props.deleteWatchlist(item.name, item.id)}}
            ]           
        )
    }

    return (
        <TouchableOpacity style={styles.groupStock}
            ...
            <Menu  style={{width:"30%", justifyContent: "center", alignItems: "flex-end", height: "100%"}}>
                ...
                <MenuOptions customStyles={{optionsContainer:{borderRadius: 10, width: "30%"}}}>
                    <MenuOption 
                        onSelect={()=>{props.handleEdit(item.name, item.id)}}>
                        <Text style={{color: "black", fontSize: 14}}>
                            Chỉnh sửa
                        </Text>
                    </MenuOption>
                    <MenuOption 
                        onSelect={handleDeleteWatchlist}>
                        <Text style={{color: "black", fontSize: 14}}>
                            Xoá
                        </Text>
                    </MenuOption>
...
const WatchlistScreen = () =>{
    ...
    const handleEdit = (name : string, id : number) => {
        ...
        setButton1({text: "Huỷ", action: handleCacel})
        setButton2({text : "Cập nhật", action : (text : string) => editWatchlist(text, id)})
        setMessage(name);
        setVisible(true);
    }
    ...
    return (
        <ScrollView>
            <AddWatchListModal title={title} visible={visible} message={message} button1={button1} button2={button2} />
            <View style={{width: windowWidth, height: "auto", alignItems: "center", backgroundColor: "#f0f0f0",}}>
                ...
                <View style={styles.listGroup}>
                    {watchlist.map((item, index)=> <GroupStock 
                    key={index} 
                    item={item} 
