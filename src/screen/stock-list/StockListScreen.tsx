...
import { arrayToGraphData, getColorPrice, getTextChangePrice } from '../../utils/utils';
...
const renderStock = (item : StockTemporary, onPress : any, index : number) => {
    ...
    const textChange = getTextChangePrice(item.priceChange, item.priceChange / item.pricePreference);
    return (
        <TouchableOpacity style={styles.stockItemContainer} key={index}
            onPress={()=> {onPress(item)}}>
            <View style={styles.stockItemContent}>
                <View style={{width: "35%", height: "100%", justifyContent: "center",}}>
                    <Text style={{color:"black", fontWeight: "600"}}>{item.shortName}</Text>
                    <Text style={{color:"#646464", fontWeight: "500"}}>Mã CP: {item.code}</Text>
                </View>
                <View style={{width:"20%",  justifyContent: 'center'}}>
                    <ShortenedGraph data={arrayToGraphData(item.timeSeries, 1)} exchange={item.exchange}
                        height={35} width={50} priceReference={item.pricePreference}/>
                </View>
                <View style={{width:"10%",  justifyContent: 'center', alignItems: 'center'}}>
                    <SMG smg={item.smg} style={{width:"80%", aspectRatio: 1}}/>
                </View>
                <View style={{width:"30%", justifyContent: 'center', alignItems:"flex-end",}}>
                    <Text style={{color:"black", fontSize: 14}}>{item.price * 1000}</Text>
...
const ListStockScreen = () =>{
    ...
    const [sortOption, setSortOption] = React.useState<{code : string, name : string}>({code : "highest_RS", name : "SMG cao nhất"});
    const [listIndustry, setListIndustry] = React.useState<Industry[]>([{industry: "Tất cả", industryId: ""}]);
    ...
    const getListStock = async (industry: string, sortBy : string , page: number, size: number, replace = false) => {
        ...
        try {
            ...
            if (res.status === 200) {
              replace? setListStock(res.data.content) 
              : 
        ...
        <View style={{flex: 1}}>
            <View style={{width: windowWidth, height: "auto", alignItems: "center", backgroundColor: "#f0f0f0",}}>
                ...
                <View style={styles.sortSettingContainer}>
                    <Text style={{color: "#5E5E5E", fontSize: 14, fontWeight: "700"}}>Sắp xếp theo: {sortOption.name}</Text>
                    <Menu>
                        <MenuTrigger>
                            <IconSort style={{width: 42, aspectRatio: 1}}/>
                        </MenuTrigger>
                        <MenuOptions customStyles={{optionsContainer:{borderRadius: 10, width:"auto"}}}>
                ...
                <View style={{width: "100%", height: 30,  flexDirection: "row", alignItems: 'center', }}>
                    <Text style={{marginLeft: 10, color: "#5E5E5E", fontSize: 14, fontWeight: "700"}}>Ngành:</Text>
                ...
                <View style={styles.listStock}>
                    <FlatList
                        ...
                        renderItem={({item, index}) => renderStock(item, handlePressItem, index)}
