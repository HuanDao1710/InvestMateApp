        ...
        if (index === 0) {
            ...
            return (
                <React.Fragment key={index}>
                    <Text style={styles.highlight}>{matches?.at(index - 1)? matches?.at(index - 1) : keyword}</Text>
                    <Text>{part}</Text>
                </React.Fragment>
            );
        }
...
const highlightText = (text : string, key : string) => {
    return (
        <HighlightedText text={text} keyword={key}/>
    )
}
        ...
        <TouchableOpacity style={{width: "100%", height: 65, borderBottomWidth: 1, flexDirection: "row", borderColor: "#DFDFDF"}}
            ...
            <View style={{height: '100%', width: "85%", alignItems: "flex-start", justifyContent: "space-around"}}>
                ...
                <Text style={{color: "black", marginLeft: 15}}>
                    Sàn: {highlightText(item.exchange, keyword)} | Mã CP: {highlightText(item.code, keyword)}
                </Text>
            </View>
