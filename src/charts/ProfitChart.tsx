...
const extractPostTaxProfitData = (
  listItem: IncomeStatementDataChartDTO[],
): any[] => {
  return listItem.map(item => ({y: item.postTaxProfit}));
};
...
const extractLineData = (
  ...
  let lineData: any[];
    ...
    lineData = listItem.map(i => (0));
  
  for (let i = 1; i < lineData.length; i++) {
    let temp1 = listItem[i - 1].postTaxProfit;
    let temp2 = listItem[i].postTaxProfit;
    if (temp1 === null || temp2 === null) continue;
    lineData[i] = (temp2 - temp1) / temp1;
  }
  return lineData.map(i => ({y: parseFloat((100 * i).toFixed(1))}));
};
...
const ProfitChart = (props: {data?: IncomeStatementDataChartDTO[]}) => {
